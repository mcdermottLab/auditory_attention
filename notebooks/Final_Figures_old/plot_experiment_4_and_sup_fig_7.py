...
if len(sys.argv) > 1:
    ...
    fig_out_dir = Path("figures/figure_3")  # Default directory
...
plt.savefig(fig_out_dir/'figure_3b_experiment_4_human_v_model_srm.pdf', transparent=True, bbox_inches='tight')
...
rm_anova_table = rm_anova(data=for_anova, dv='threshold_snr', subject='model', within=["distractor_azim"], effsize='np2')
print("Experiment 4: ANOVA for model")
...
models = ['Feature-gain Model', 'Baseline CNN', 'Early-only', 'Late-only']
...
plt.savefig(fig_out_dir/'sup_figure_7.pdf', transparent=True, bbox_inches='tight')
