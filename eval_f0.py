...
def run_eval(args):
    ...
    if snr == 'clean':
        log_name = f"{model_name}_{snr}_f0_test"
    else:
        log_name = f"{model_name}_{snr}dB_SNR_f0_test"
...
def cli_main():
    ...
    parser.add_argument(
        "--eval_cond_file",
        default=pathlib.Path("/om2/user/imgriff/projects/End-to-end-ASR-Pytorch/attn_cnn_snr_eval_conditions.pkl"),
        type=pathlib.Path,
        help="Directory to save checkpoints and logs to. (Default: './exp')",
    )
    parser.add_argument(
