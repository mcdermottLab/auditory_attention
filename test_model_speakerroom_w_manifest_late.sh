...
which python3
python3 eval_binaural_w_manifest.py --config config/binaural_attn/word_task_late_only_v08.yaml \
                 --ckpt_path attn_cue_models/word_task_late_only_v08/checkpoints/epoch=3-step=53756.ckpt  \
                 --location_manifest /om2/user/imgriff/projects/torch_2_aud_attn/binaural_test_manifests/match_human_pilot_conds.pkl \
                 --model_name word_task_late_only_v08 --location_idx $SLURM_ARRAY_TASK_ID \
                 --gpus 1 --n_jobs 2 --exp_dir binaural_eval/human_pilot_conds \
                 --cue_type voice_and_location --overwrite
