...
def get_activations(args):
    ...
    set_bg_noise_level = at.BinauralRMSNormalizeForegroundAndBackground(50).cuda() # set level of bg noise to 50 dB SPL
    bg_noise_brir  = SpatializeAllLocs(diffuse_brirs, model_sr=sr).cuda()
    # get dataset
        ...
        with torch.no_grad():
            ...
            for loc_x, (azim, elev) in enumerate(tqdm(loc_pairs, desc="Location ")):
                ...
                for ix, batch in tqdm(enumerate(dataloader),  total = n_activations, desc=f'Processing activations for location {azim}az {elev}elev', leave=False):
                    ...
                    nat_scene_dist, _ = set_bg_noise_level(nat_scene_dist.cuda(), None)
                    nat_scene_dist = bg_noise_brir(nat_scene_dist)

                    # get mixture signals 
                    mixture, _ = audio_transforms(target, nat_scene_dist)
                    ...
                    activations = {}
                    gain_shape_dict = {}
                    model(None, mixture, None)  # None is cue_mask_ixs which is not used for activations
