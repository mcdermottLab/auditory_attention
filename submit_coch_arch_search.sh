...
#SBATCH --output=outLogs/%A/cochlear_word_rec_%A_%a.out
#SBATCH --error=outLogs/%A/cochlear_word_rec_%A_%a.err
#SBATCH --mem=32Gb
...
python3 train.py --config model_files/${rel_date}_coch_model_${layer_depth}-layered_config_${SLURM_ARRAY_TASK_ID}.json --gpus 2 --n_jobs 10 --mixed_precision \
                 --exp_dir ./arch_search_results/${rel_date}_${SLURM_JOB_ID}/jsin_precombined_gammatone_40_channels_20kHz_1e-4lr_${SLURM_ARRAY_TASK_ID} \
