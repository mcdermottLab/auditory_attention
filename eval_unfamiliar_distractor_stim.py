...
def run_eval(args):
    ...
    test_manifest = pickle.load(open(args.test_manifest, 'rb'))

    language, snr = test_manifest[args.array_id]
    print(f"Evaluating {model_name} on {language} distractors at {snr}db SNR")
    ...
    dataset = SWCMonoTestSetUnfamililarLanguage(manifest_path=args.stim_manifest_path,
                                                model_sr=sr,
    ...
    def collate_fn(batch):
        ...
        mixtures = torch.stack([audio_transforms(targ, dist)[0] for _, targ, dist,  _ in batch])
    ...
    with open(out_dir / f"{model_name}_{language}_distractor_{snr}dB_SNR_eval_results.csv", 'w') as file:
        ...
        # run eval 
        for i, batch in enumerate(tqdm(dataloader, desc=f"evaluating {model_name} on {language} distractor at {snr}dB SNR")):
...
def cli_main():
    ...
    parser.add_argument(
        "--test_manifest",
        default=pathlib.Path("./exp.pkl"),
        type=pathlib.Path,
        help="Path to test condition mannifest ",
    )
    parser.add_argument(
        "--stim_manifest_path",
        default=pathlib.Path("/om/user/imgriff/datasets/human_distractor_language_2024/final_stim_manifest_w_cue_tg_lang_dists.pdpkl"),
        type=pathlib.Path,
