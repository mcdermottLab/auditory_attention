...
#SBATCH --array=1-60 # 0-60 for standard test
...
python3 eval_swc_mono_stim.py --config config/binaural_attn/word_task_v10_backbone_word_config_saddler_dataset_learned_gains.yaml \
                 --ckpt_path attn_cue_models/word_task_v10_backbone_word_config_saddler_dataset_learned_gains/checkpoints/epoch=0-step=8000.ckpt \
                 --array_id $SLURM_ARRAY_TASK_ID \
                 --n_jobs 4 --exp_dir swc_2024_eval_full_stim/ \
                 --stim_path /om/user/imgriff/datasets/human_word_rec_SWC_2024/model_eval_stim.h5 \
                 --stim_cond_map binaural_test_manifests/swc_all_cond_h5_job_manifest.pkl \
