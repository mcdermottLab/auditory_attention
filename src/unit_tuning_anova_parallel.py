...
def main(args):
    ...
    # Filter for conv_block layers with relu_mixture
    all_keys = list(h5.keys())
    layer_names = [key for key in all_keys if isinstance(h5[key], h5py.Dataset) 
                   and 'conv_block' in key and 'relu_mixture' in key]
    ...
    prop_var_per_unit = np.zeros((n_units_this_job, 3))
    ssq_per_unit = np.zeros((n_units_this_job, 3))
    ...
    out_name = out_path / f"{layer_name}_anova_results_units_{start_unit:05d}_{end_unit-1:05d}.npz"
