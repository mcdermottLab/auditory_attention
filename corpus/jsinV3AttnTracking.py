...
class jsinV3_attn_tracking(torch.utils.data.ConcatDataset):
    ...
    def __init__(self, root, mode='train', download=False, transform=None, noise_only=False):
        """
...
class H5Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        talker_ix = speakers[index]

        # get cue 
        cue_ixs = np.where(speakers[:] == talker_ix)[0]
        cue_ixs = cue_ixs[cue_ixs != index] # don't include target excerpt
        cue_ix = np.random.choice(cue_ixs)
        assert speakers[cue_ix] == talker_ix, "Cue selected from different talker!"
